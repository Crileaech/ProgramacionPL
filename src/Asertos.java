...
public class Asertos extends AnasintBaseVisitor<Boolean>{
    ...
    public Boolean existencial(Anasint.AsertosContext ctx) {
        ...
        Interprete.asig.put(var, comienzo);
        while((Integer) Interprete.asig.get(var)<=(Integer)fin) {
            Boolean res = (Boolean) evalua.visit(cuant.expr_bool());
            if(res) { return true; }
            Interprete.asig.put(var,(Integer) Interprete.asig.get(var)+1);
        }
        return false;
    }
    ...
    public Boolean universal(Anasint.AsertosContext ctx) {
        ...
        Interprete.asig.put(var, comienzo);
        Boolean res = (Boolean) evalua.visit(cuant.expr_bool());
        while(res && (Integer) Interprete.asig.get(var)<=(Integer)fin) {
