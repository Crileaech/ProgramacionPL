...
public class Compilador extends AnasintBaseListener {
    ...
    public void gencodigo_asignacion(ParseTree var, Anasint.ExprContext expr){
        String txt_expr="0";
        String v;
        v = var.getText();
        if (expr != null)
            txt_expr = generador.visit(expr);
        gencode_espacios();

        try {
            fichero.write(v + "=" + txt_expr + ";\n"); //código de una asignación
        } catch (IOException e) {
            e.getMessage();
        }
    }

    public void gencodigo_asignacion_funciones(ParseTree var, Anasint.ExprContext expr, int num){
        String txt_expr="0";
        String v;
        v = var.getText();
        if (expr != null)
...
// Genera código procedimientos
    ...
    public void enterSecuencias(Anasint.SecuenciasContext ctx){
        if(aux_subprogramas ==0){
            ...
            if(ctx.tipos_no_elementales().getChild(0)==ctx.tipos_no_elementales().SEQ_NUM()){
                declarar_variable_Seq_Integer_Temporal(ctx.getChild(0).getText());
            }
            if(ctx.tipos_no_elementales().getChild(0)==ctx.tipos_no_elementales().SEQ_BOOL()){
                declarar_variable_Seq_Bool_Temporal(ctx.getChild(0).getText());
            }
        }
    }
    ...
    public void enterAsig(Anasint.AsigContext ctx){
        if(ruptura_condicional==0){
            if(ctx.asignacion().variable().size()>1 && ctx.asignacion().expr(0).getChild(0).getChild(0).getClass()==Anasint.Expr_funcContext.class){
                //; \ln
                int suma = ctx.asignacion().variable().size() + ctx.asignacion().expr_sacar_elem().size();
                int pos = 0;
                Anasint.ExprContext expr = ctx.asignacion().expr(0);
                for (int i = 0; i < (suma * 2 - 1); i = i + 2) { // Por las comas
                    gencodigo_asignacion_funciones(ctx.asignacion().getChild(i), expr, pos);
                    pos++;
                }
            }else{
                int suma = ctx.asignacion().variable().size() + ctx.asignacion().expr_sacar_elem().size();
